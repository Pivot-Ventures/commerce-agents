...
def test_admin_approves_and_rejects_and_refuses_payouts(client):
    ...
    desk = client.get("/api/admin/haulage-desk", headers=headers).json()
    assert desk["queue"] and desk["agents"]
    attached = client.post(
        "/api/admin/hires/HIRE-7821/attach-agent",
        json={"agent_id": "AG-11"},
        headers=headers,
    )
    assert attached.status_code == 200
    assert attached.json()["agent_id"] == "AG-11"
    shipping = client.post("/api/admin/hires/HIRE-7821/pay-shipping", json={}, headers=headers)
